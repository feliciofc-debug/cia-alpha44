...
// Uso: node tools/inspect-xlsx.mjs "<caminho>" [maxRows]
import * as XLSX from "xlsx";
import { argv } from "node:process";

const file = argv[2];
const maxRows = Number(argv[3] ?? 25);

if (!file) {
  console.error("Uso: node tools/inspect-xlsx.mjs <arquivo> [maxRows]");
  process.exit(1);
}
...
for (const name of wb.SheetNames) {
  ...
  slice.forEach((r, i) => {
    // remove células totalmente vazias no fim
