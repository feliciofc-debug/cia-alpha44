...
export interface DetectarPrecoCustoInput {
  descOriginal: string;
  descPt?: string | null;
  /** Texto/justificativa gerada pela visão quando a foto foi usada na classificação. */
  descVisao?: string | null;
  ncm?: string | null;
  uso?: string | null;
...
function tipoPorTexto(desc: string): TipoPrecoCusto | null {
...
function ncmSinalVeiculo(ncm: string | null | undefined): boolean {
  const key = ncm8(ncm);
  return key.startsWith("8711");
}
...
/**
 * Detecta moto ou patinete elétrico completo por consenso conservador:
 * NCM capítulo 8711, texto fornecedor/PT e visão contam como sinais independentes.
 */
export function detectarPrecoCusto(
  ...
  const descOriginal = ctx.descOriginal.trim();
  const descTexto = [descOriginal, ctx.descPt?.trim()].filter(Boolean).join(" ");
  if (!descTexto) return null;
  if (RE_USO_PECA.test(descTexto)) return null;

  const descVisao = ctx.descVisao?.trim() ?? "";
  const tipoDescricao = tipoPorTexto(descTexto);
  const tipoVisao = descVisao ? tipoPorTexto(descVisao) : null;
  const tipoNcm = tipoPorNcm(ctx.ncm, descTexto);

  const sinais = [
    ncmSinalVeiculo(ctx.ncm),
    Boolean(tipoDescricao),
    Boolean(tipoVisao),
  ].filter(Boolean).length;
  if (sinais < 2) return null;

  const tipo = tipoDescricao ?? tipoVisao ?? tipoNcm;
  if (!tipo) return null;
...
export function avisoPrecoCusto(tipo: TipoPrecoCusto): string {
  return `Base FOB = valor de custo da unidade (veículo: ${rotuloPrecoCusto(tipo)}, US$ ${precoCustoUnitarioUSD(tipo)}/un).`;
}
